...
export const getAutoComplete: RequestHandler = async (req, res) => {
  try {
...
export const getAutoCompleteInCity: RequestHandler = async (req, res) => {
  try {
    if (!req.query.input) {
    ...
    let responseModel: Array<any> = await Promise.all(
      citys.predictions.map(async (e: any) => {
        try {
